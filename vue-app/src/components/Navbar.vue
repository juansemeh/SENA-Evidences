  ...
  <Disclosure as="nav" class="bg-gray-800 fixed top-0 left-0 w-full z-50" v-slot="{ open }">
        ...
        <div class="flex flex-6 items-center justify-center sm:items-stretch sm:justify-start">
          ...
          <div class="hidden sm:ml-6 sm:block">
            <div class="flex space-x-4">
              <Menu v-for="category in categories" :key="category" as="div" class="relative inline-block text-left">
                <MenuButton class="inline-flex w-full justify-center rounded-md bg-gray-800 px-3 py-2 text-sm font-medium text-gray-300 hover:bg-gray-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
                  {{ category }}
                ...
                <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                  <MenuItems class="absolute left-0 z-10 mt-2 w-56 origin-top-left rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div class="py-1">
                      <MenuItem v-for="exercise in exercises[category]" :key="exercise.href" v-slot="{ active }">
                        <a href="#" @click.prevent="onSelect(exercise)" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'block px-4 py-2 text-sm']">
                          {{ exercise.name }}
                        </a>
                      </MenuItem>
                    </div>
          ...
          <Menu as="div" class="relative ml-3">
            <div>
              <MenuButton class="relative flex rounded-full bg-gray-800 text-sm focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800 focus:outline-hidden">
                ...
                <img class="size-8 rounded-full" src="https://www.svgrepo.com/show/350417/user-circle.svg" alt="" />
              </MenuButton>
            </div>
            <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
    ...
    <DisclosurePanel class="sm:hidden">
      <div class="space-y-1 px-2 pt-2 pb-3">
        <Disclosure v-for="category in categories" :key="category" as="div" class="mb-1">
          <DisclosureButton class="w-full text-left rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-gray-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
            {{ category }}
          </DisclosureButton>
          <DisclosurePanel class="space-y-1 pl-4">
            <a v-for="exercise in exercises[category]" :key="exercise.href" href="#" @click.prevent="onSelect(exercise)" class="block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-gray-700 hover:text-white">
              {{ exercise.name }}
            </a>
          </DisclosurePanel>
        </Disclosure>
